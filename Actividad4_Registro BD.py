...
def guardar_alumno():
    ...
    print(int_age)
    print(name.get())
    conexion.execute("insert into alumnos(nombre, edad, Ciudad, Matricula) values (?,?)", (name.get(), int_age))
    conexion.commit()
    conexion.close()
...
def nuevo_alumno():
    ...
    age_label = tk.Label(ventana_nuevo_alumno, text="Ciudad:")
    age_label.grid(row=2, column=0, padx=(10,0))

    age_entry = tk.Entry(ventana_nuevo_alumno)
    age_entry.grid(row=2, column=1, padx=(0, 10))

    age_label = tk.Label(ventana_nuevo_alumno, text="Matricula:")
    age_label.grid(row=3, column=0, padx=(10,0))

    age_entry = tk.Entry(ventana_nuevo_alumno)
    age_entry.grid(row=3, column=1, padx=(0, 10))
    ...
    submit_button.grid(row=4, column=0, columnspan=4, pady=10, padx=10)
...
menu_alumnos.add_command(label="Agregar Datos del Alumno", accelerator="Ctrl+N", command=nuevo_alumno)
